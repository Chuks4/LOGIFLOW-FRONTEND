...
export default function DriverDashboard() {
  ...
  const columns: TableColumn<Shipment>[] = [
    ...
    },
    {
      ...
      render: (shipment) => shipment.deliveryAddress,
    },
    {
      key: "status",
      header: "Status",
  ...
  return (
    <section className={styles.dashboard}>
      <div className={styles.welcome}>
        <p className={styles.kicker}>Driver operations</p>
        <h2>Your delivery route at a glance.</h2>
        <p>Review assigned shipments and keep delivery progress up to date.</p>
      </div>
      <div className={styles.stats}>
        {[
          ["Assigned", assigned, "Shipments waiting for pickup"],
          ["In progress", inTransit, "Picked up or in transit"],
          ["Delivered", delivered, "Completed deliveries"],
          ["Total assigned", totalItems, "Shipments assigned to you"],
        ].map(([label, value, note]) => (
          <article className={styles.card} key={label as string}>
            <p>{label}</p>
            <strong>{value}</strong>
            <span>{note}</span>
          </article>
      ...
      <div className={styles.shipments}>
        <div className={styles.shipmentsHeader}>
          <p className={styles.kicker}>Delivery queue</p>
          <h3>Shipments assigned to you</h3>
        </div>
